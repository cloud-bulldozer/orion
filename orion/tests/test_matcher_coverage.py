...
class TestParseAggResults:
    def test_no_aggregations_returns_empty(self, matcher):
        ...
        metrics = {"metric_of_interest": "cpu", "agg": {"agg_type": "avg"}}
        result = matcher.parse_agg_results(fake_data, "avg", metrics=metrics)
        assert result == []
...
class TestGetAggMetricQuery:
    def _make_agg_response(self, metric_of_interest, agg_type, value=42.0):
        """Build a fake aggregation response keyed by metric_of_interest."""
        if agg_type == "percentiles":
            agg_data = {metric_of_interest: {"values": {"95.0": value}}}
        else:
            agg_data = {metric_of_interest: {"value": value}}
    ...
    def test_avg_agg_type(self, matcher, monkeypatch):
        ...
        resp = self._make_agg_response("cpuUsage", "avg", 85.5)
        ...
        assert len(result) == 1
        assert result[0]["cpuUsage_avg"] == 85.5

    def test_sum_agg_type(self, matcher, monkeypatch):
        """Sum aggregation type."""
        resp = self._make_agg_response("bytes", "sum", 1000.0)
        monkeypatch.setattr(Search, "execute", lambda self: resp)
        metrics = {
            "name": "throughput",
        ...
        result = matcher.get_agg_metric_query(["test-uuid"], metrics)
        assert result[0]["bytes_sum"] == 1000.0

    def test_max_agg_type(self, matcher, monkeypatch):
        """Max aggregation type."""
        resp = self._make_agg_response("latMs", "max", 99.9)
        monkeypatch.setattr(Search, "execute", lambda self: resp)
        metrics = {
            "name": "latency",
        ...
        result = matcher.get_agg_metric_query(["test-uuid"], metrics)
        assert result[0]["latMs_max"] == 99.9

    def _make_multi_percentile_response(self, metric_of_interest, percentile_map):
        """Build a fake aggregation response with multiple percentile values."""
        agg_data = {metric_of_interest: {"values": percentile_map}}
        return Response(response={
            "hits": {"hits": [], "total": {"value": 0, "relation": "eq"}},
            "aggregations": {
                "uuid": {
    ...
    def test_percentiles_agg_type(self, matcher, monkeypatch):
        ...
        resp = self._make_agg_response("latMs", "percentiles", 12.5)
        monkeypatch.setattr(Search, "execute", lambda self: resp)
        metrics = {
            "name": "latency",
            "metric_of_interest": "latMs",
        ...
        result = matcher.get_agg_metric_query(["test-uuid"], metrics)
        assert result[0]["latMs_percentiles_95.0"] == 12.5

    def test_percentiles_no_target_returns_all(self, matcher, monkeypatch):
        """Without target_percentile, all percentile values are returned."""
        pct_values = {"50.0": 100.5, "95.0": 250.3, "99.0": 350.7}
        resp = self._make_multi_percentile_response("latMs", pct_values)
        monkeypatch.setattr(Search, "execute", lambda self: resp)
        metrics = {
            "name": "latency",
            "metric_of_interest": "latMs",
        ...
        result = matcher.get_agg_metric_query(["test-uuid"], metrics)
        assert len(result) == 1
        assert result[0]["latMs_percentiles_50.0"] == 100.5
        assert result[0]["latMs_percentiles_95.0"] == 250.3
        assert result[0]["latMs_percentiles_99.0"] == 350.7

    def test_percentiles_no_target_no_agg_key(self, matcher, monkeypatch):
        """Without target_percentile, all percentiles returned with different field."""
        pct_values = {"10.0": 5.0, "90.0": 95.0}
        resp = self._make_multi_percentile_response("v", pct_values)
        monkeypatch.setattr(Search, "execute", lambda self: resp)
        metrics = {
            "name": "metric",
        ...
        result = matcher.get_agg_metric_query(["test-uuid"], metrics)
        assert result[0]["v_percentiles_10.0"] == 5.0
        assert result[0]["v_percentiles_90.0"] == 95.0

    def test_percentiles_target_missing_from_response(self, matcher, monkeypatch):
        """target_percentile requested but not in response values returns None."""
        pct_values = {"50.0": 100.5, "95.0": 250.3}
        resp = self._make_multi_percentile_response("latMs", pct_values)
        monkeypatch.setattr(Search, "execute", lambda self: resp)
        metrics = {
            "name": "latency",
        ...
        result = matcher.get_agg_metric_query(["test-uuid"], metrics)
        assert result[0]["latMs_percentiles_99.0"] is None

    def test_count_agg_type(self, matcher, monkeypatch):
        """Count aggregation uses value_count."""
        resp = self._make_agg_response("eventId", "count", 42)
        monkeypatch.setattr(Search, "execute", lambda self: resp)
        metrics = {
            "name": "events",
        ...
        result = matcher.get_agg_metric_query(["test-uuid"], metrics)
        assert result[0]["eventId_count"] == 42

    def test_custom_timestamp_field(self, matcher, monkeypatch):
        """Custom timestamp field used in sort and aggregation."""
        resp = self._make_agg_response("cpuUsage", "avg", 50.0)
        captured = {}
