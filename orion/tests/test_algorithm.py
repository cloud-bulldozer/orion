...
class TestEDivisive:
    ...
    def test_is_acked(self):
        ...
        assert algo._is_acked(ack_set, "throughput", [cp], 0) is True
        assert algo._is_acked(set(), "throughput", [cp], 0) is False
...
class TestCMR:
    ...
    def test_combine_non_numeric_columns_joined(self):
        ...
        algo = CMR(df, _make_test(), _make_options(), _make_metrics_config())
        result = algo.combine_and_average_runs(df)
        # uuid column should be from most recent baseline run (uuid-1)
    ...
    def test_cmr_json_output_reduces_rows(self):
        """CMR combine_and_average reduces rows; JsonFormatter should reflect that."""
        ...
        result = AnalysisResult(
            ...
            test=_make_test(),
            dataframe=algo.dataframe,
            metrics_config=_make_metrics_config(),
            change_points_by_metric=cps,
            series=series,
            regression_flag=False,
            avg_values=algo.dataframe[list(_make_metrics_config().keys())].mean(),
            collapse=False,
            display_fields=[],
            column_group_size=5,
            uuid_field="uuid",
        ...
        records = json.loads(formatted["test-bench"])
