...
class TestShortenUrlsBatch:
    def test_single_url(self, utils, monkeypatch):
        mock_resp = MagicMock()
        mock_resp.text = "https://tinyurl.com/abc\n"
        mock_resp.raise_for_status = MagicMock()
        mock_session = MagicMock()
        mock_session.get.return_value = mock_resp
        monkeypatch.setattr("orion.utils.requests.Session", lambda: mock_session)
        result = utils.shorten_urls_batch({"uuid1": "https://example.com/long"})
        assert result == {"uuid1": "https://tinyurl.com/abc"}

    def test_multiple_urls(self, utils, monkeypatch):
        def fake_get(url, params=None, timeout=None):
            resp = MagicMock()
            resp.raise_for_status = MagicMock()
            short_map = {
                "https://example.com/1": "https://tinyurl.com/a",
                "https://example.com/2": "https://tinyurl.com/b",
            }
            resp.text = short_map[params["url"]]
            return resp
        mock_session = MagicMock()
        ...
        assert result == {
            "uuid1": "https://tinyurl.com/a",
            "uuid2": "https://tinyurl.com/b",
        }
...
class TestGetMetricData:
    ...
    def test_exception_in_metric_skips_it(self, utils):
        mock_match = MagicMock()
        mock_match.get_results_batch.side_effect = RuntimeError("batch failed")
    ...
    def test_metric_config_preserves_direction_and_threshold(self, utils):
        ...
        _, config, __ = utils.get_metric_data(
            ["u1"], metrics, mock_match, test_threshold=5
        )
