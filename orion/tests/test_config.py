...
class TestParsePullNumber:
    def test_valid_positive_int(self):
        assert _parse_pull_number(42) == 42

    def test_valid_string(self):
        assert _parse_pull_number("123") == 123

    def test_zero_is_valid(self):
        assert _parse_pull_number(0) == 0

    def test_negative_raises(self):
        with pytest.raises(ValueError, match="invalid pull number"):
            _parse_pull_number(-1)

    def test_non_numeric_string_raises(self):
        with pytest.raises(ValueError, match="invalid pull number"):
            _parse_pull_number("abc")

    def test_none_raises(self):
        with pytest.raises(ValueError, match="invalid pull number"):
            _parse_pull_number(None)

    def test_float_string_raises(self):
        with pytest.raises(ValueError, match="invalid pull number"):
            _parse_pull_number("1.5")
...
class TestReplacePlaceholders:
    def test_single_var(self):
        assert _replace_placeholders("hello ${name}", {"name": "world"}) == "hello world"

    def test_multiple_vars(self):
        result = _replace_placeholders("${a}-${b}", {"a": "x", "b": "y"})
        assert result == "x-y"

    def test_unmatched_placeholder_left_as_is(self):
        assert _replace_placeholders("${missing}", {}) == "${missing}"

    def test_no_placeholders(self):
        assert _replace_placeholders("plain text", {"x": "y"}) == "plain text"

    def test_dotted_var_name(self):
        assert _replace_placeholders("${a.b}", {"a.b": "val"}) == "val"

    def test_non_string_value_converted(self):
        assert _replace_placeholders("count=${n}", {"n": 42}) == "count=42"
...
class TestSubstituteVars:
    def test_dict_string_values(self):
        obj = {"name": "${x}", "other": "no-var"}
        _substitute_vars(obj, {"x": "replaced"})
        assert obj["name"] == "replaced"
        assert obj["other"] == "no-var"

    def test_nested_dict(self):
        obj = {"outer": {"inner": "${v}"}}
        _substitute_vars(obj, {"v": "done"})
        assert obj["outer"]["inner"] == "done"

    def test_list_values(self):
        obj = ["${a}", "static", "${b}"]
        _substitute_vars(obj, {"a": "1", "b": "2"})
        assert obj == ["1", "static", "2"]

    def test_nested_list_in_dict(self):
        obj = {"items": ["${x}", {"deep": "${x}"}]}
        _substitute_vars(obj, {"x": "val"})
        assert obj["items"][0] == "val"
        assert obj["items"][1]["deep"] == "val"

    def test_non_string_values_unchanged(self):
        obj = {"num": 42, "flag": True}
        _substitute_vars(obj, {"num": "99"})
        assert obj["num"] == 42
...
class TestExpandFanOut:
    def test_no_fan_out_passthrough(self):
        metrics = [{"name": "cpu", "metric_of_interest": "value"}]
        result = expand_fan_out(metrics, SingletonLogger.get_logger("Orion"))
        assert result == [{"name": "cpu", "metric_of_interest": "value"}]

    def test_fan_out_expands(self):
        metrics = [{
            "name": "latency-${label}",
            "metricName": "api_latency",
            "metric_of_interest": "value",
        ...
        result = expand_fan_out(metrics, SingletonLogger.get_logger("Orion"))
        assert len(result) == 2
        assert result[0]["name"] == "latency-p50"
        assert result[1]["name"] == "latency-p99"

    def test_fan_out_field_override(self):
        metrics = [{
            "name": "m",
            "threshold": 10,
            "metric_of_interest": "value",
    ...
    def test_mixed_fan_out_and_plain(self):
        metrics = [
            {"name": "plain", "metric_of_interest": "v"},
            {
                "name": "tmpl-${x}",
                "metric_of_interest": "v",
                "fan_out": [{"x": "a"}],
            },
        ]
        result = expand_fan_out(metrics, SingletonLogger.get_logger("Orion"))
        ...
        assert result[1]["name"] == "tmpl-a"

    def test_fan_out_deep_copies(self):
        metrics = [{
            "name": "m",
            "nested": {"key": "${v}"},
            "metric_of_interest": "val",
            "fan_out": [{"v": "1"}, {"v": "2"}],
        }]
        result = expand_fan_out(metrics, SingletonLogger.get_logger("Orion"))
        assert result[0]["nested"]["key"] == "1"
        assert result[1]["nested"]["key"] == "2"
...
class TestExpandGroupBy:
    def _mock_matcher(self, values):
        matcher = MagicMock()
        matcher.discover_field_values.return_value = values
        return matcher

    def test_no_group_by_passthrough(self):
        metrics = [{"name": "cpu", "metric_of_interest": "v"}]
        matcher = self._mock_matcher([])
        result = expand_group_by(metrics, matcher, ["uuid1"], SingletonLogger.get_logger("Orion"))
        assert result == metrics
        matcher.discover_field_values.assert_not_called()

    def test_group_by_expands(self):
        metrics = [{"name": "m-${node}", "group_by": "node", "metric_of_interest": "v"}]
        matcher = self._mock_matcher(["worker-0", "worker-1"])
        result = expand_group_by(metrics, matcher, ["u1"], SingletonLogger.get_logger("Orion"))
        assert len(result) == 2
        assert result[0]["node"] == "worker-0"
        assert result[0]["name"] == "m-worker-0"
        assert result[1]["node"] == "worker-1"

    def test_group_by_list_single_field(self):
        metrics = [{"name": "m", "group_by": ["region"], "metric_of_interest": "v"}]
        matcher = self._mock_matcher(["us-east", "eu-west"])
        result = expand_group_by(metrics, matcher, ["u1"], SingletonLogger.get_logger("Orion"))
        assert len(result) == 2

    def test_group_by_multi_field_exits(self):
        metrics = [{"name": "m", "group_by": ["a", "b"], "metric_of_interest": "v"}]
        matcher = self._mock_matcher([])
        with pytest.raises(SystemExit):
            expand_group_by(metrics, matcher, ["u1"], SingletonLogger.get_logger("Orion"))

    def test_group_by_no_values_skipped(self):
        metrics = [{"name": "m", "group_by": "field", "metric_of_interest": "v"}]
        matcher = self._mock_matcher([])
        result = expand_group_by(metrics, matcher, ["u1"], SingletonLogger.get_logger("Orion"))
        assert result == []

    def test_mixed_group_by_and_plain(self):
        metrics = [
            {"name": "plain", "metric_of_interest": "v"},
            {"name": "grouped", "group_by": "zone", "metric_of_interest": "v"},
        ]
        matcher = self._mock_matcher(["az1"])
        result = expand_group_by(metrics, matcher, ["u1"], SingletonLogger.get_logger("Orion"))
        assert len(result) == 2
        assert result[0]["name"] == "plain"
        assert result[1]["zone"] == "az1"
