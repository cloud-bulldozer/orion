...
def extract_metadata_from_test(test: Dict[str, Any]) -> Dict[Any, Any]:
    ...
    metadata[version_field] = str(metadata[version_field])
...
def process_test(
    ...
    for df in dataframe_list:
        if "timestamp" in df.columns:
            uuid_timestamp_map = pd.concat(
                [uuid_timestamp_map, df[[uuid_field, "timestamp"]].drop_duplicates()]
            )
    uuid_timestamp_map = uuid_timestamp_map.drop_duplicates(subset=[uuid_field])
    ...
    merged_df = merged_df.merge(uuid_timestamp_map, on=uuid_field, how="left")
